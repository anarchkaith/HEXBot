...
function resolveFooterContact(report, reporter) {
...
function buildDirectiveValue(report) {
  const analysis = report.analysis || {};
  const tacticalDirective = getFallback(analysis.tacticalDirective, '');
  if (tacticalDirective) {
    return truncateText(
      tacticalDirective,
      DISCORD_LIMITS.fieldValue,
      'Staff: Validar evidencia y aplicar sancion. Miembros: Evitar contacto y reportar actividad.'
    );
  }

  const operational = getFallback(
    analysis.operationalRecommendation || analysis.recommendation,
    'Aplicar seguimiento y escalar segun evidencia.'
  );

  return truncateText(
    `Staff: ${operational}\nMiembros: Evitar contacto, guardar evidencia y reportar por /report.`,
    DISCORD_LIMITS.fieldValue,
    'Staff: Validar evidencia y aplicar sancion. Miembros: Evitar contacto y reportar actividad.'
  );
}
...
function buildMostWantedReportMessage({
  ...
  const footerContact = resolveFooterContact(report, reporter);
  ...
  const tacticalDirective = buildDirectiveValue(report);
  ...
  const embed = new EmbedBuilder()
    ...
    .setTitle(truncateText('TARGET MARCADO', DISCORD_LIMITS.title))
  ...
  const fields = [
    ...
    },
    {
      ...
      value: truncateText(recommendation, DISCORD_LIMITS.fieldValue, 'Sin recomendacion operativa disponible.')
    },
    {
      name: '🧭 DIRECTIVA BREVE',
